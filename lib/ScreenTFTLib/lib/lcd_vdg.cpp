...
#include <ScreenTFT/lcd_vdg_fontes.h>  

unsigned int pBaseVideoMem[11][320][240];  // ~2MB Video Memory
unsigned int pBaseVideoMem2[844800];  // ~2MB Video Memory

CLcdVdg *CLcdVdg::s_pThis = 0;

CLcdVdg::CLcdVdg (void)
{
    s_pThis = this;
}
...
boolean CLcdVdg::Initialize (void)
{
    TFT_Init();
...
void CLcdVdg::TFT_Init(void)
{
    ...
    lcd_vdg_Delayms(5);   // 5mS
    bcm2835_gpio_write(TFT_RST,0);
    lcd_vdg_Delayms(15);  // 15mS
    bcm2835_gpio_write(TFT_RST,1);
    lcd_vdg_Delayms(15);  // 15mS
...
//-------------------------------------------------------------------
int CLcdVdg::commVDG(unsigned char *vparam)
{
...
//-------------------------------------------------------------------
int CLcdVdg::TCHVerif(unsigned char pRetAds, Coordinate screen_cal) {
	int iRetAux = 0;

	if (pRetAds && screen_cal.x != 0 && screen_cal.y != 0) {
		ptec[1] = 0x00;
		ptec[2] = 0x00;
...
//-------------------------------------------------------------------
void CLcdVdg::Write_Bytes_GPIO(unsigned int wByte)
{
	unsigned int tByte;
...
//-------------------------------------------------------------------
void CLcdVdg::Write_Command(unsigned int wcommand)
{
  	bcm2835_gpio_write(TFT_RD,1);
...
//-------------------------------------------------------------------
void CLcdVdg::Write_Data(unsigned int wdata)
{
...
//-------------------------------------------------------------------
unsigned int CLcdVdg::Read_Data(void) {
	// Ler memoria de video em RAM interna
    return 0;
}

//-------------------------------------------------------------------
void CLcdVdg::Write_Command_Data(unsigned int wcommand,unsigned int Wdata)
{
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Set_Address(unsigned int PX1,unsigned int PY1,unsigned int PX2,unsigned int PY2)
{
    pBaseVideoAddrX = PY1;
    pBaseVideoAddrY = PX1;
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Fill(unsigned int color)
{
  unsigned int i,j;
  bcm2835_gpio_write(TFT_CS,0);
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Box(unsigned int X1,unsigned int Y1,unsigned int X2,unsigned int Y2,unsigned int color)
{
  unsigned int  i,j;
  bcm2835_gpio_write(TFT_CS,0);
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Dot(unsigned int x,unsigned int y,unsigned int color)
{
  bcm2835_gpio_write(TFT_CS,0);
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Line(unsigned int X1,unsigned int Y1,unsigned int X2,unsigned int Y2,unsigned int color)
{
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_H_Line(char X1,char X2,unsigned int y_pos,unsigned int color)
{
    TFT_Box(X1,y_pos,X2,y_pos+1,color);
}

//-------------------------------------------------------------------
void CLcdVdg::TFT_V_Line(unsigned int Y1,unsigned int Y2,char x_pos,unsigned int color)
{
    TFT_Box(x_pos,Y1,x_pos+1,Y2,color);
}

//-------------------------------------------------------------------
void CLcdVdg::TFT_Rectangle(unsigned int X1,unsigned int Y1,unsigned int X2,unsigned int Y2,unsigned int color)
{
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Circle(unsigned int x,unsigned int y,char radius,char fill,unsigned int color)
{
 int a_,b_,P;
 a_ = 0;
 b_ = radius;
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Text(char* S,unsigned int x,unsigned int y,char DimFont,unsigned int Fcolor,unsigned int Bcolor)
{
    while (*S)
    {
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_InvertRect(unsigned int pos_x,unsigned int pos_y,unsigned int dim_x,unsigned int dim_y) 
{
...
//-------------------------------------------------------------------
void CLcdVdg::TFT_Scroll(unsigned char qtdlin) 
{
...
//-------------------------------------------------------------------
//-------------------------------------------------------------------
void CLcdVdg::ReadKeyMatrix(unsigned char vini, unsigned char vlin, unsigned char vqtd) 
{
	unsigned char vkey;
...
//-------------------------------------------------------------------
void CLcdVdg::HideKeyboard(unsigned int xposini, unsigned int yposini) 
{
	pkeyativo = 0x00;
...
//-------------------------------------------------------------------
void CLcdVdg::ShowKeyboard(unsigned int xposini, unsigned int yposini, unsigned char vnewkey) 
{
...
void CLcdVdg::ReturnKeyboard(unsigned int xposini, unsigned int yposini, unsigned int vpostx, unsigned int vposty) 
{
	...
	if (ptec[1] && vpx != 0 && vpy != 0) {
		...
		lcd_vdg_Delayms(200);	// +/- 200mS
		TFT_InvertRect(vpy, vpx, 16, 16);
	}
}

void CLcdVdg::lcd_vdg_Delayns(long pTime)
{
    long ra;

    for(ra=0;ra<pTime;ra++) asm("nop");
}

void CLcdVdg::lcd_vdg_Delayms(int pTime)
{
    long ra;

    for(ra=0;ra<(pTime * 5000);ra++) asm("nop");
}
