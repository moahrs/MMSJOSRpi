...
CIRCLELIB = ../../CircleLib
BCM2835LIB = ../../Bcm2835Lib

-include $(LIBHOME)/Config.mk
...
INCLUDE	+= -I $(LIBHOME)/include -I $(CIRCLELIB)/include -I $(BCM2835LIB)/include

AFLAGS	+= $(ARCH) -DRASPPI=$(RASPPI) -DSTDLIB_SUPPORT=$(STDLIB_SUPPORT) $(INCLUDE) $(OPTIMIZE)
CFLAGS	+= $(ARCH) -mcpu=arm1176jzf-s -fpic -ffreestanding \
...
$(TARGET).img: $(OBJS) $(LIBS) $(LIBHOME)/lib/startup.o $(LIBHOME)/circle.ld
	$(LD) -o $(TARGET).elf -Map $(TARGET).map -T $(LIBHOME)/circle.ld $(LIBHOME)/lib/startup.o $(OBJS) $(EXTRALIBS) $(LIBS) $(EXTRALIBS)
...
ifneq ($(strip $(REBOOTMAGIC)),)
	python $(LIBHOME)/tools/reboottool.py $(REBOOTMAGIC) $(SERIALPORT) $(USERBAUD)
endif
	python $(LIBHOME)/tools/flasher.py $(TARGET).hex $(SERIALPORT) $(FLASHBAUD)
