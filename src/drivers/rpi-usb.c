...
#include "drivers/rpi-smartstart.h"		// Provides timing routines and mailbox routines to power up/down the USB.  
...
RESULT EnumerateDevice(struct UsbDevice *device, struct UsbDevice* ParentHub, uint8_t PortNum) {
	...
	LOG_DEBUG("Device: %i, Class: %i\n", 0 /*device->Pipe0.Number*/, 0 /*device->Descriptor.Class*/ );
